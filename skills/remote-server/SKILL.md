---
name: remote-server
description: "Piteronline remote server management and troubleshooting. AUTO-CONSULT when: piteronline is down, remote service issues, nginx/PHP/MariaDB errors, SSL problems on piteronline, or healthcheck reports remote issues."
---

# Remote Server (Piteronline)

Manage piteronline.tv via SSH. All operations use server_remote and server_inspect tools.
...
| php84-php-fpm | PHP application server | 9000 (unix socket) |
...
3. If PHP-FPM: check for stuck processes
   server_remote(action: "logs", service: "php84-php-fpm", lines: 100)
4. If MariaDB: check slow queries
   server_remote(action: "logs", service: "mariadb", lines: 100)
```
...
2. Check WordPress debug.log size
...
1. **mariadb** — database must be first
...
- Never restart mariadb without warning — active queries will be killed
