...
func registerDevMode(server *mcp.Server, agent *engine.ServerAgent) {
	mcp.AddTool(server, &mcp.Tool{
		...
		text, err := handleDevMode(ctx, agent, input)
		if err != nil {
...
func handleDevMode(_ context.Context, agent *engine.ServerAgent, input engine.DevModeInput) (string, error) {
	ttl := 4 * time.Hour
	if input.TTL != "" {
		d, err := time.ParseDuration(input.TTL)
		if err != nil {
			return "", fmt.Errorf("invalid ttl %q: %w", input.TTL, err)
		}
		ttl = d
	}
